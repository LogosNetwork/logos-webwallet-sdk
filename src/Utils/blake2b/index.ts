...
const ADD64AA = (v:Uint32Array, a:number, b:number) => {
  const o0 = v[a] + v[b]
  let o1 = v[a + 1] + v[b + 1]
  if (o0 >= 0x100000000) {
    o1++
...
const ADD64AC = (v:Uint32Array, a:number, b0:number, b1:number) => {
  let o0 = v[a] + b0
  if (b0 < 0) {
    o0 += 0x100000000
  }
...
const blake2bCompress = (ctx:Context, last:boolean) => {
  let i = 0
...
export const initalizeBlake2b = () => {
  return new Promise<boolean|Error>((reject, resolve) => {
...
export default class Blake2b {
  ...
  private parameter_block: Uint8Array
  ...
  constructor (outlen: number = 32, key: Uint8Array = null, salt: Uint8Array = null, personal: Uint8Array = null) {
    ...
    if (wasm && wasm.exports && (this.mode === null || this.mode === 'wasm')) {
      ...
      this.parameter_block[0] = outlen
      if (key) this.parameter_block[1] = key.length
      ...
      for (let i = 0; i < 16; i++) {
        this.context.h[i] = BLAKE2B_IV32[i] ^ B2B_GET32(this.parameter_block, i * 4)
      }
  ...
  update = (input: Uint8Array | Buffer) => {
  ...
  digest = (out?: ('binary'|'hex')|(Uint8Array|Buffer)) => {
    if (this.finalized) throw new Error(`Hash instance finalized`)
    if (wasm) {
  ...
  final = this.digest
  WASM = wasm && wasm.buffer
}

// Updates a BLAKE2b streaming hash
// Requires hash context and Uint8Array (byte array)
const blake2bUpdate = (ctx: Context, input: Uint8Array) => {
...
const blake2bFinal = (ctx: Context, out: Uint8Array) => {
...
const blake2bDigest = (ctx: Context, out?: ('binary'|'hex')|Uint8Array) => {
  let buf = (!out || out === 'binary' || out === 'hex') ? new Uint8Array(ctx.outlen) : out
...
const hexSlice = (buf: Buffer|Uint8Array) => {
  let str = ''
  for (let i = 0; i < buf.length; i++) str += toHex(buf[i])
  return str
}

const wasmHexSlice = (buf: Buffer|Uint8Array, start: number, len: number) => {
  let str = ''
...
let WASM_LOADED = () => Boolean(wasm && wasm.exports)
const BYTES_MIN = 16
const BYTES_MAX = 64
const BYTES = 32
